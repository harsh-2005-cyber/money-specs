...
if page == "Add Expense":
    ...
    st.write(f"This week: {this_week_start.strftime('%Y-%m-%d')} to {today.strftime('%Y-%m-%d')}, Records: {len(this_week_expenses) if this_week_expenses is not None else 0}")
    st.write(f"Last week: {last_week_start.strftime('%Y-%m-%d')} to {last_week_end.strftime('%Y-%m-%d')}, Records: {len(last_week_expenses) if last_week_expenses is not None else 0}")
